...
func New[K cmp.Ordered, V any]() *Tree[K, V] {
	return &Tree[K, V]{Comparator: cmp.Compare[K], rWLock: &sync.RWMutex{}}
}

// NewWith instantiates a red-black tree with the custom comparator.
func NewWith[K comparable, V any](comparator utils.Comparator[K]) *Tree[K, V] {
	return &Tree[K, V]{Comparator: comparator, rWLock: &sync.RWMutex{}}
}
...
// Key should adhere to the comparator's type assertion, otherwise method panics.
func (tree *Tree[K, V]) Remove(key K) (value V, found bool) {
	tree.rWLock.Lock()
	defer tree.rWLock.Unlock()
	var child *Node[K, V]
	node := tree.lookup(key)
